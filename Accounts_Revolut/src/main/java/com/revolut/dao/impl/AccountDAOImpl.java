...
public class AccountDAOImpl implements AccountDAO {
	 ...
	 * Get all accounts.
	 */
	public List<Account> getAccounts() throws CustomException {
		...
		try {
			...
			while (rs.next()) {
				...
				if (log.isDebugEnabled())
					log.debug("getAccounts(): Get  Account " + acc);
				accounts.add(acc);
			}
			return accounts;
	 ...
	 */
	public int updateBalance(long accountId, BigDecimal deltaAmount) throws CustomException {
		Connection conn = null;
		...
		try {
			...
			if (rs.next()) {
				targetAccount = new Account(rs.getLong("AccountId"), rs.getString("UserName"),
						rs.getBigDecimal("Balance"), rs.getString("CurrencyCode"));
				if (log.isDebugEnabled())
					log.debug("updateBalance from Account: " + targetAccount);
			}
			...
			log.error("updateBalance(): User Transaction Failed, rollback initiated for: " + accountId, se);
			try {
				if (conn != null)
					conn.rollback();
